...
impl Fabric {
    pub fn new(attr: &crate::FabricAttr) -> Self {
        let mut c_fabric: *mut libfabric_sys::fid_fabric  = std::ptr::null_mut();
        let mut c_fabric_ptr: *mut *mut libfabric_sys::fid_fabric = &mut c_fabric;

        let err = unsafe {libfabric_sys::fi_fabric(attr.c_attr, c_fabric_ptr, std::ptr::null_mut())};
        if err != 0 {
