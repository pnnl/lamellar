...
pub struct EpCompletionOwnedTypedFid<FID: AsRawFid> {
    pub(crate) typed_fid: OwnedTypedFid<FID>,
    pub(crate) bound_cq0: std::sync::OnceLock<std::sync::Arc<parking_lot::RwLock<TypedFid<CqRawFid>>>>,
    pub(crate) bound_cq1: std::sync::OnceLock<std::sync::Arc<parking_lot::RwLock<TypedFid<CqRawFid>>>>,
    pub(crate) bound_cntr: std::sync::OnceLock<std::sync::Arc<parking_lot::RwLock<TypedFid<CntrRawFid>>>>,
}
...
impl<FID: AsRawFid + AsRawTypedFid> OwnedTypedFid<FID> {
    pub(crate) fn from(typed_fid: FID) -> Self {
        #[cfg(any(feature="threading-fid", feature="threading-domain", feature="threading-completion"))]
        return Self { typed_fid: std::sync::Arc::new(parking_lot::RwLock::new(TypedFid(typed_fid))) };
        #[cfg(not(any(feature="threading-fid", feature="threading-domain", feature="threading-completion")))]
...
impl<FID: AsRawFid + AsRawTypedFid> DomainOwnedTypedFid<FID> {
    pub(crate) fn from(typed_fid: FID, domain: std::sync::Arc<parking_lot::Mutex<TypedFid<DomainRawFid>>>) -> Self {
        return Self { 
...
impl<FID: AsRawFid + AsRawTypedFid> XContextOwnedTypedFid<FID> {
    pub(crate) fn from(typed_fid: FID, parent_ep: std::sync::Arc<parking_lot::RwLock<TypedFid<EpRawFid>>>) -> Self {
        return Self { 
            typed_fid: OwnedTypedFid::from(typed_fid),
            parent_ep, 
        };
...
impl<'a, FID: AsRawFid + AsRawTypedFid<Output = FID> > AsRawTypedFid for MutBorrowedTypedFid<'a, FID> {
    type Output = FID;
    #[inline]
    fn as_raw_typed_fid(&self) -> Self::Output {
        match self {
...
impl<FID: AsRawFid > AsTypedFid<FID> for  XContextOwnedTypedFid<FID> {
    ...
    fn as_typed_fid_mut(&self) -> MutBorrowedTypedFid<'_, FID> {
        MutBorrowedTypedFid::XContext(

            MutXContextBorrowedTypedFid {
...
impl<FID: AsRawFid > AsTypedFid<FID> for  DomainOwnedTypedFid<FID> {
    fn as_typed_fid(&self) -> BorrowedTypedFid<'_, FID> {
        BorrowedTypedFid::Domain(

            DomainBorrowedTypedFid {
    ...
    fn as_typed_fid_mut(&self) -> MutBorrowedTypedFid<'_, FID> {
        MutBorrowedTypedFid::Domain(

            MutDomainBorrowedTypedFid {
                _domain_fid: self.domain.lock(),
                typed_fid: self.typed_fid.typed_fid.write(),
                phantom: PhantomData,
            }
...
impl<FID: AsRawFid > AsTypedFid<FID> for  EpCompletionOwnedTypedFid<FID> {
    fn as_typed_fid(&self) -> BorrowedTypedFid<'_, FID> {
        BorrowedTypedFid::Completion(

            EpCompletionBorrowedTypedFid {
                // _domain_fid: self.domain.lock(),
                typed_fid: self.typed_fid.typed_fid.read(),
                phantom: PhantomData,
            }
    ...
    fn as_typed_fid_mut(&self) -> MutBorrowedTypedFid<'_, FID> {
        MutBorrowedTypedFid::Completion(

            MutEpCompletionBorrowedTypedFid {
