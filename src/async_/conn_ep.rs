...
use crate::{
    conn_ep::{ConnectionPendingEndpointBase, EnabledConnectionOrientedEndpoint}, ep::{Address, Connected, EndpointBase, EndpointImplBase, PendingAccept, Unconnected, UninitUnconnected}, eq::{ConnectedEvent, Event, EventQueueBase}, fid::{AsRawFid, AsRawTypedFid, AsTypedFid, Fid}, utils::check_error
};
...
impl<E> UninitUnconnectedEndpointBase<EndpointImplBase<E, dyn AsyncReadEq, dyn AsyncReadCq>> {
    pub fn enable<EQ: AsyncReadEq + 'static>(self, eq: &EventQueueBase<EQ>) -> Result<EnabledConnectionOrientedEndpoint<EndpointImplBase<E, dyn AsyncReadEq, dyn AsyncReadCq>>, crate::error::Error> {
        self.bind_eq(eq)?;
        let err =
        ...
        if self.inner.has_conn_req {
            Ok(
                EnabledConnectionOrientedEndpoint::AcceptPending(
            ...
            Ok(
                EnabledConnectionOrientedEndpoint::Unconnected (
                    UnconnectedEndpoint {
                        inner: self.inner.clone(),
                        phantom: PhantomData,
                    }
...
pub type ConnectionPendingEndpoint<T> = ConnectionPendingEndpointBase<EndpointImplBase<T, dyn AsyncReadEq, dyn AsyncReadCq>>;
