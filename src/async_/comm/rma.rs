...
pub(crate) trait AsyncReadEpImpl: AsyncTxEp + ReadEpImpl {
    async unsafe  fn read_async_impl<T>(&self, buf: &mut [T], desc: &mut impl DataDescriptor, src_mapped_addr: Option<&MappedAddress>, mem_addr: u64,  mapped_key: &MappedMemoryRegionKey, user_ctx: Option<*mut std::ffi::c_void>) -> Result<SingleCompletion, crate::error::Error> {
        ...
        self.read_impl(buf, desc, src_mapped_addr, mem_addr, mapped_key, Some(&mut async_ctx as *mut AsyncCtx as *mut std::ffi::c_void))?;
    ...
    async unsafe fn readv_async_impl<'a>(&self, iov: &[crate::iovec::IoVecMut<'a>], desc: &mut [impl DataDescriptor], src_mapped_addr: Option<&MappedAddress>, mem_addr: u64, mapped_key: &MappedMemoryRegionKey, user_ctx: Option<*mut std::ffi::c_void>) -> Result<SingleCompletion, crate::error::Error> { 
        let mut async_ctx = AsyncCtx{user_ctx};
        self.readv_impl(iov, desc, src_mapped_addr, mem_addr, mapped_key, Some(&mut async_ctx as *mut AsyncCtx as *mut std::ffi::c_void))?;
        let cq = self.retrieve_tx_cq();
        cq.wait_for_ctx_async(&mut async_ctx).await
    }

    async unsafe  fn readmsg_async_impl<'a>(&self, mut msg: Either<&mut crate::msg::MsgRmaMut<'a>, &mut crate::msg::MsgRmaConnectedMut<'a>>, options: ReadMsgOptions) -> Result<SingleCompletion, crate::error::Error> {
        ...
        let imm_msg = match msg {
            Either::Left(ref mut msg) => {msg.get_mut().context = (&mut async_ctx as *mut AsyncCtx).cast(); Either::<&MsgRmaMut, &MsgRmaConnectedMut>::Left(msg)},
            Either::Right(ref mut msg) => {msg.get_mut().context = (&mut async_ctx as *mut AsyncCtx).cast(); Either::<&MsgRmaMut, &MsgRmaConnectedMut>::Right(msg)},
        };
...
pub(crate) trait AsyncWriteEpImpl: AsyncTxEp + WriteEpImpl {
    async unsafe fn write_async_impl<T>(&self, buf: &[T], desc: &mut impl DataDescriptor, dest_mapped_addr: Option<&MappedAddress>, mem_addr: u64, mapped_key: &MappedMemoryRegionKey, user_ctx: Option<*mut std::ffi::c_void>) -> Result<SingleCompletion, crate::error::Error>  {
        let mut async_ctx = AsyncCtx{user_ctx};
        self.write_impl(buf, desc, dest_mapped_addr, mem_addr, mapped_key, Some(&mut async_ctx as *mut AsyncCtx as *mut std::ffi::c_void))?;
        let cq = self.retrieve_tx_cq();
        cq.wait_for_ctx_async(&mut async_ctx).await
    }
    ...
    async unsafe fn writev_async_impl<'a>(&self, iov: &[crate::iovec::IoVec<'a>], desc: &mut [impl DataDescriptor], dest_mapped_addr: Option<&MappedAddress>, mem_addr: u64, mapped_key: &MappedMemoryRegionKey,  user_ctx: Option<*mut std::ffi::c_void>) -> Result<SingleCompletion, crate::error::Error> { 
        let mut async_ctx = AsyncCtx{user_ctx};
        self.writev_impl(iov, desc, dest_mapped_addr, mem_addr, mapped_key, Some(&mut async_ctx as *mut AsyncCtx as *mut std::ffi::c_void))?;
        let cq = self.retrieve_tx_cq();
        cq.wait_for_ctx_async(&mut async_ctx).await
    }
    ...
    async unsafe  fn writedata_async_impl<T>(&self, buf: &[T], desc: &mut impl DataDescriptor, data: u64, dest_mapped_addr: Option<&MappedAddress>, mem_addr: u64, mapped_key: &MappedMemoryRegionKey, user_ctx: Option<*mut std::ffi::c_void>) -> Result<SingleCompletion, crate::error::Error> { 
        let mut async_ctx = AsyncCtx{user_ctx};
        self.writedata_impl(buf, desc, data, dest_mapped_addr, mem_addr, mapped_key, Some(&mut async_ctx as *mut AsyncCtx as *mut std::ffi::c_void))?;
        let cq = self.retrieve_tx_cq();
        cq.wait_for_ctx_async(&mut async_ctx).await
    }

    async unsafe  fn writemsg_async_impl<'a>(&self, mut msg: Either<&mut crate::msg::MsgRma<'a>, &mut crate::msg::MsgRmaConnected<'a>>, options: WriteMsgOptions) -> Result<SingleCompletion, crate::error::Error> {
        let real_user_ctx = match msg {
            Either::Left(ref mut msg) => msg.get_mut().context,
            Either::Right(ref mut msg) => msg.get_mut().context,
        };
        ...
        let imm_msg = match msg {
            Either::Left(ref mut msg) => {msg.get_mut().context = (&mut async_ctx as *mut AsyncCtx).cast(); Either::<&MsgRma, &MsgRmaConnected>::Left(msg)},
            Either::Right(ref mut msg) => {msg.get_mut().context = (&mut async_ctx as *mut AsyncCtx).cast(); Either::<&MsgRma, &MsgRmaConnected>::Right(msg)},
        };
...
impl<E: AsyncWriteEpImpl> AsyncWriteEpImpl for EndpointBase<E> {
    ...
    async unsafe  fn writemsg_async_impl<'a>(&self, msg: Either<&mut crate::msg::MsgRma<'a>, &mut crate::msg::MsgRmaConnected<'a>>, options: WriteMsgOptions) -> Result<SingleCompletion, crate::error::Error> {
...
impl<EP: AsyncWriteEpImpl + ConnectedEp> ConnectedAsyncWriteEp for EP {
    ...
    #[inline]
    async unsafe  fn writemsg_async<'a>(&self, msg: &mut crate::msg::MsgRmaConnected<'a>, options: WriteMsgOptions) -> Result<SingleCompletion, crate::error::Error> {
