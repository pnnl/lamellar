...
impl DomainBase<dyn AsyncReadEq> {
    ...
    pub(crate) fn bind_eq<EQ: AsyncReadEq + 'static>(
        &self,
...
impl<'a, E> DomainBuilder<'a, E> {
    pub fn build_and_bind_async<EQ: AsyncReadEq + 'static>(
        self,
        eq: &EventQueue<EQ>,
        async_mem_reg: bool,
    ) -> Result<DomainBase<dyn AsyncReadEq>, crate::error::Error> {
        let domain = DomainBase::<dyn AsyncReadEq>::new(
            self.fabric,
            self.info,
            self.flags,
