...
impl<EP: AsTypedFid<EpRawFid>> AsRawTypedFid for EnabledConnectionOrientedEndpoint<EP> {
    ...
    fn as_raw_typed_fid(&self) -> Self::Output {
        match self {
            EnabledConnectionOrientedEndpoint::Unconnected(ep) => {
                ep.as_typed_fid().as_raw_typed_fid()
            }
            EnabledConnectionOrientedEndpoint::AcceptPending(ep) => {
                ep.as_typed_fid().as_raw_typed_fid()
            }
        }
    }
...
impl<E> UninitUnconnectedEndpointBase<EndpointImplBase<E, dyn ReadEq, dyn ReadCq>> {
    ...
    pub fn enable<EQ: ReadEq + 'static>(
        ...
        EnabledConnectionOrientedEndpoint<EndpointImplBase<E, dyn ReadEq, dyn ReadCq>>,
        crate::error::Error,
    > {
        self.bind_eq(eq)?;
        let err =
...
impl<EP: AsTypedFid<EpRawFid>> AcceptPendingEndpointBase<EP> {
    ...
    pub fn accept_with<T0>(
        self,
        param: &[T0],
    ) -> Result<ConnectionPendingEndpointBase<EP>, crate::error::Error> {
        let err = unsafe {
...
pub type ConnectionPendingEndpoint<T> =
    ConnectionPendingEndpointBase<EndpointImplBase<T, dyn ReadEq, dyn ReadCq>>;
