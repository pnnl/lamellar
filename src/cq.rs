use std::{marker::PhantomData, os::fd::{AsFd, BorrowedFd}, rc::Rc, cell::{RefCell, Ref}};
...
pub type MsgEntry = libfabric_sys::fi_cq_entry;
pub type DataEntry = libfabric_sys::fi_cq_entry;
pub type TaggedEntry = libfabric_sys::fi_cq_entry;
pub type UnspecEntry = ();


pub(crate) struct CompletionQueueImpl<Format: EntryFormat> {
    c_cq: *mut libfabric_sys::fid_cq,
    entry_buff: RefCell<Vec<Completion<Format>>>,
...
/// are also dropped.
pub struct CompletionQueue<T: CqConfig, Format: EntryFormat> {
    pub(crate) inner: Rc<CompletionQueueImpl<Format>>,
    phantom: PhantomData<T>,
}

impl<'a, Format: EntryFormat> CompletionQueueImpl<Format> {
        ...
        if err != 0 {
            ...
            Ok(
                Self {
                    ...
                    phantom: PhantomData,
                    entry_buff: RefCell::new(Vec::with_capacity(default_buff_size)),
    ...
    pub(crate) fn read(&self, count: usize) -> Result<Vec<Completion<Format>>, crate::error::Error> {
        let err = {
            let mut borrowed_entries = self.entry_buff.borrow_mut();
            read_cq_entry!(libfabric_sys::inlined_fi_cq_read, self.format, self.handle(),count, Completion<Format>, borrowed_entries, )
        };
        if err < 0 {
            Err(crate::error::Error::from_err_code((-err).try_into().unwrap()) ) 
        }
    ...
    pub(crate) fn read_in(&self, count: usize, buffer: &mut [Completion<Format>]) -> Result<(), crate::error::Error> {

        let err  = read_cq_entry_in_slice!(libfabric_sys::inlined_fi_cq_read, self.format, self.handle(), count, Completion<Format>, buffer, );
        if err < 0 {
            Err(crate::error::Error::from_err_code((-err).try_into().unwrap()) ) 
        }
        else {
    ...
    pub(crate) fn readfrom(&self, count: usize) -> Result<(Vec<Completion<Format>>, Option<MappedAddress>), crate::error::Error> {
       
        let mut address = 0;
        let p_address = &mut address as *mut libfabric_sys::fi_addr_t;    
        let err = {
            let mut borrowed_entries = self.entry_buff.borrow_mut();
            read_cq_entry!(libfabric_sys::inlined_fi_cq_readfrom, self.format, self.handle(),count, Completion<Format>, borrowed_entries, p_address)
        };
    ...
    pub(crate) fn readfrom_in(&self, count: usize, buffer: &mut [Completion<Format>]) -> Result<Option<MappedAddress>, crate::error::Error> {
       
        let mut address = 0;
        let p_address = &mut address as *mut libfabric_sys::fi_addr_t;    
        let err = read_cq_entry_in_slice!(libfabric_sys::inlined_fi_cq_readfrom, self.format, self.handle(), count, Completion<Format>, buffer, p_address);
        let address = if address == crate::FI_ADDR_NOTAVAIL {
            None
        }
        else {
    ...
    pub(crate) fn sread(&self, count: usize, cond: usize, timeout: i32) -> Result<Vec<Completion<Format>>, crate::error::Error> {
        let p_cond = cond as *const usize as *const std::ffi::c_void;
        let err =  {
            let mut borrowed_entries = self.entry_buff.borrow_mut();
            read_cq_entry!(libfabric_sys::inlined_fi_cq_sread, self.format, self.handle(), count, Completion<Format>, borrowed_entries, p_cond, timeout)
        };
        if err < 0 {
            Err(crate::error::Error::from_err_code((-err).try_into().unwrap()) ) 
        }
    ...
    pub(crate) fn sread_in(&self, count: usize, buffer: &mut [Completion<Format>], cond: usize, timeout: i32) -> Result<(), crate::error::Error> {
        let p_cond = cond as *const usize as *const std::ffi::c_void;
        let err = read_cq_entry_in_slice!(libfabric_sys::inlined_fi_cq_sread, self.format, self.handle(), count, Completion<Format>, buffer, p_cond, timeout);
        if err < 0 {
            Err(crate::error::Error::from_err_code((-err).try_into().unwrap()) ) 
        }
        else {
    ...
    pub(crate) fn sreadfrom(&self, count: usize, cond: usize, timeout: i32) -> Result<(Vec<Completion<Format>>, Option<MappedAddress>), crate::error::Error> {
        ...
        let err = {
            let mut borrowed_entries = self.entry_buff.borrow_mut();
            read_cq_entry!(libfabric_sys::inlined_fi_cq_sreadfrom, self.format, self.handle(), count, Completion<Format>, borrowed_entries, p_address, p_cond, timeout)
        };
        let address = if address == crate::FI_ADDR_NOTAVAIL {
            None
        }
    ...
    pub(crate) fn sreadfrom_in(&self, count: usize, buffer: &mut [Completion<Format>], cond: usize, timeout: i32) -> Result< Option<MappedAddress>, crate::error::Error> {
        
        let p_cond = cond as *const usize as *const std::ffi::c_void;
        let mut address = 0;
        let p_address = &mut address as *mut RawMappedAddress;   
        let err = read_cq_entry_in_slice!(libfabric_sys::inlined_fi_cq_sreadfrom, self.format, self.handle(), count, Completion<Format>, buffer, p_address, p_cond, timeout);
        let address = if address == crate::FI_ADDR_NOTAVAIL {
            None
        }
        else {
...
impl<T: CqConfig, Format: EntryFormat> CompletionQueue<T, Format> {
    #[allow(dead_code)]
    pub(crate) fn handle(&self) -> *mut libfabric_sys::fid_cq {
        self.inner.handle()
    }
    ...
    pub fn read(&self, count: usize) -> Result<Vec<Completion<Format>>, crate::error::Error> {
    ...
    /// 
    /// Corresponds to `fi_cq_read` with the `buf` maintained and casted automatically
    pub fn read_in(&self, count: usize, buff: &mut [Completion<Format>]) -> Result<(), crate::error::Error> {
    ...
    /// 
    /// Corresponds to `fi_cq_read` with the `buf` maintained and casted automatically
    pub unsafe fn read_in_unchecked(&self, count: usize, buff: &mut [Completion<Format>]) -> Result<(), crate::error::Error> {
        self.inner.read_in(count, buff)
    }
    ...
    pub fn readfrom(&self, count: usize) -> Result<(Vec<Completion<Format>>, Option<MappedAddress>), crate::error::Error> {
    ...
    /// 
    /// Corresponds to `fi_cq_readfrom`
    pub fn readfrom_in(&self, count: usize, buff: &mut [Completion<Format>]) -> Result<Option<MappedAddress>, crate::error::Error> {
        assert!(count <= buff.len());
    ...
    /// 
    /// Corresponds to `fi_cq_readfrom`
    pub unsafe fn readfrom_in_unchecked(&self, count: usize, buff: &mut [Completion<Format>]) -> Result<Option<MappedAddress>, crate::error::Error> {
        self.inner.readfrom_in(count, buff)
    }
...
impl<T: CqConfig + Waitable, Format: EntryFormat> CompletionQueue<T, Format> {
    ...
    pub fn sread(&self, count: usize, timeout: i32) -> Result<Vec<Completion<Format>>, crate::error::Error> {
    ...
    /// 
    /// Corresponds to `fi_cq_sread` with `cond` set to `NULL`.
    pub fn sread_in(&self, count: usize, buff: &mut [Completion<Format>], timeout: i32) -> Result<(), crate::error::Error> {
        assert!(count <= buff.len());
    ...
    /// 
    /// Corresponds to `fi_cq_sread` with `cond` set to `NULL`.
    pub unsafe fn sread_in_unchecked(&self, count: usize, buff: &mut [Completion<Format>], timeout: i32) -> Result<(), crate::error::Error> {
        self.inner.sread_in(count, buff, 0, timeout)
    }
    ...
    pub fn sread_with_cond(&self, count: usize, cond: usize, timeout: i32) -> Result<Vec<Completion<Format>>, crate::error::Error> {
    ...
    /// 
    /// Corresponds to `fi_cq_sread`
    pub fn sread_with_cond_in(&self, count: usize, buff: &mut [Completion<Format>], cond: usize, timeout: i32) -> Result<(), crate::error::Error> {
        assert!(count <= buff.len());
    ...
    pub fn sreadfrom(&self, count: usize, timeout: i32) -> Result<(Vec<Completion<Format>>, Option<MappedAddress>), crate::error::Error> {
    ...
    /// 
    /// Corresponds to `fi_cq_sreadfrom` with `cond` set to `NULL`.
    pub fn sreadfrom_in(&self, count: usize, buff: &mut [Completion<Format>], timeout: i32) -> Result<Option<MappedAddress>, crate::error::Error> {
        assert!(count <= buff.len());
    ...
    /// 
    /// Corresponds to `fi_cq_sreadfrom` with `cond` set to `NULL`.
    pub unsafe fn sreadfrom_in_unchecked(&self, count: usize, buff: &mut [Completion<Format>], timeout: i32) -> Result<Option<MappedAddress>, crate::error::Error> {
        self.inner.sreadfrom_in(count, buff, 0, timeout)
    }
    ...
    pub fn sreadfrom_with_cond(&self, count: usize, cond: usize, timeout: i32) -> Result<(Vec<Completion<Format>>, Option<MappedAddress>), crate::error::Error> {
    ...
    /// 
    /// Corresponds to `fi_cq_sreadfrom`
    pub fn sreadfrom_with_cond_in(&self, count: usize, buff: &mut [Completion<Format>], cond: usize, timeout: i32) -> Result<Option<MappedAddress>, crate::error::Error> {
        assert!(count <= buff.len());
    ...
    /// 
    /// Corresponds to `fi_cq_sreadfrom`
    pub unsafe fn sreadfrom_with_cond_in_unchecked(&self, count: usize, buff: &mut [Completion<Format>], cond: usize, timeout: i32) -> Result<Option<MappedAddress>, crate::error::Error> {
        self.inner.sreadfrom_in(count, buff, cond, timeout)
    }
...
impl<'a, T: CqConfig + WaitRetrievable, Format: EntryFormat> CompletionQueue<T, Format> { //[TODO] Make this a method of the trait ?
...
impl<Format: EntryFormat> crate::BindImpl for CompletionQueueImpl<Format> {}

impl<T: CqConfig + 'static, Format: EntryFormat+ 'static> crate::Bind for CompletionQueue<T, Format> {
    fn inner(&self) -> Rc<dyn crate::BindImpl> {
        self.inner.clone()
    }
}

impl<Format: EntryFormat> AsFid for CompletionQueueImpl<Format> {
    fn as_fid(&self) -> crate::fid::BorrowedFid<'_> {
        self.fid.as_fid()
    }
}

impl<T: CqConfig, Format: EntryFormat> AsFid for CompletionQueue<T, Format> {
    fn as_fid(&self) -> crate::fid::BorrowedFid<'_> {
        self.inner.as_fid()
    }
}

impl<Format: EntryFormat> AsFd for CompletionQueueImpl<Format> {
...
impl<T: CqConfig + WaitRetrievable + FdRetrievable, Format: EntryFormat> AsFd for CompletionQueue<T, Format> {
    fn as_fd(&self) -> BorrowedFd<'_> {
        self.inner.as_fd()
    }
}
...
pub struct CompletionQueueBuilder<'a, T, WAIT, WAITFD, Format> {
...
impl<'a, T, WAIT, WAITFD, Format: EntryFormat> CompletionQueueBuilder<'a, T, WAIT,  WAITFD, Format> {
    ...
    pub fn wait_none(mut self) -> CompletionQueueBuilder<'a, T, cqoptions::WaitNone, cqoptions::Off, Format> {
    ...
    pub fn wait_fd(mut self) -> CompletionQueueBuilder<'a, T, cqoptions::WaitRetrieve, cqoptions::On, Format> {
    ...
    pub fn wait_set(mut self, set: &crate::sync::WaitSet) -> CompletionQueueBuilder<'a, T, cqoptions::WaitNoRetrieve, cqoptions::Off, Format> {
    ...
    pub fn wait_mutex(mut self) -> CompletionQueueBuilder<'a, T, cqoptions::WaitRetrieve, cqoptions::Off, Format> {
    ...
    pub fn wait_yield(mut self) -> CompletionQueueBuilder<'a, T, cqoptions::WaitNoRetrieve, cqoptions::Off, Format> {
    ...
    pub fn context(self, ctx: &'a mut T) -> CompletionQueueBuilder<'a, T, WAIT, WAITFD, Format> {
        CompletionQueueBuilder {
            ctx: Some(ctx),
            cq_attr: self.cq_attr,
            domain: self.domain,
    ...
    pub fn build(self) ->  Result<CompletionQueue<Options<WAIT, WAITFD>, Format>, crate::error::Error> {
