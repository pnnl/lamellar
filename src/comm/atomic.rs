...
impl<E: AtomicCap+ WriteMod, EQ: EventQueueImplT, CQ: ?Sized + CompletionQueueImplT> EndpointBase<E, EQ, CQ> {
    ...
    pub fn inject_atomic<T: 'static>(&self, buf: &[T], dest_addr: &crate::MappedAddress, mem_addr: u64, mapped_key: &MappedMemoryRegionKey, op: crate::enums::Op) -> Result<(), crate::error::Error>{
        let err = unsafe{ libfabric_sys::inlined_fi_inject_atomic(self.as_raw_typed_fid(), buf.as_ptr().cast(), buf.len(), dest_addr.raw_addr(), mem_addr, mapped_key.get_key(), to_fi_datatype::<T>(), op.get_value())};
        check_error(err)
    }
    ...
    pub fn inject_atomic_connected<T: 'static>(&self, buf: &[T], mem_addr: u64, mapped_key: &MappedMemoryRegionKey, op: crate::enums::Op) -> Result<(), crate::error::Error>{
        let err = unsafe{ libfabric_sys::inlined_fi_inject_atomic(self.as_raw_typed_fid(), buf.as_ptr().cast(), buf.len(), FI_ADDR_UNSPEC, mem_addr, mapped_key.get_key(), to_fi_datatype::<T>(), op.get_value())};
        
        check_error(err)
    }
}

impl<E: AtomicCap+ ReadMod + WriteMod, EQ: EventQueueImplT, CQ: ?Sized + CompletionQueueImplT> EndpointBase<E, EQ, CQ> {
