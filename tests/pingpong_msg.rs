...
fn pp_server_msg() {
    ...
    let (info, fab, eq, _pep) = common::start_server::<common::EventQueueOptions>(hints, "172.17.110.7".to_owned(), "9222".to_owned());
...
fn pp_client_msg() {
    ...
    let (info, _fab, domain, _eq, rx_cq, tx_cq, tx_cntr, rx_cntr, ep, _mr, mut mr_desc) = 
        common::ft_client_connect(hints, &mut gl_ctx, "172.17.110.7".to_owned(), "9222".to_owned());
    let entries = info.get();
    let test_sizes = gl_ctx.test_sizes.clone();
    for msg_size in test_sizes {
