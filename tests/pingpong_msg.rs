...
fn pp_server_msg() {
    ...
    let (info, fab, eq, _pep) = common::start_server::<common::EventQueueOptions>(hints, "172.17.110.9".to_owned(), "9222".to_owned());
    let (domain, tx_cq, rx_cq, tx_cntr, rx_cntr, ep, _mr, mut mr_desc) = common::ft_server_connect(&mut gl_ctx, &eq, &fab);
...
fn pp_client_msg() {
    ...
    let (info, _fab, domain, _eq, rx_cq, tx_cq, tx_cntr, rx_cntr, ep, _mr, mut mr_desc) = 
        common::ft_client_connect(hints, &mut gl_ctx, "172.17.110.9".to_owned(), "9222".to_owned());
    let entries = info.get();
    let test_sizes = gl_ctx.test_sizes.clone();
    for msg_size in test_sizes {
