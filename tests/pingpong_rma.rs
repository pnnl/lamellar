...
define_test!(pp_server_rma, async_pp_server_rma, {
    ...
    let (infocap, ep, domain, cq_type, tx_cntr, rx_cntr, mut mr, _av, mut mr_desc) = 
        call!(prefix::ft_init_fabric, hintscaps, &mut gl_ctx, "".to_owned(), "9222".to_owned(), true);

    match infocap {
        prefix::InfoWithCaps::Msg(entry) => {
            
            let remote = call!(prefix::ft_exchange_keys, &entry, &mut gl_ctx, mr.as_mut().unwrap(), &cq_type, &tx_cntr, &rx_cntr,&domain, &ep, &mut mr_desc);

            let test_sizes = gl_ctx.test_sizes.clone();
            for msg_size in test_sizes {
                call!(prefix::pingpong_rma, &entry, &mut gl_ctx, &cq_type, &tx_cntr, &rx_cntr,&ep, &mut mr_desc, prefix::RmaOp::RMA_WRITE, &remote, 100, 10, msg_size, true);
            }
...
define_test!(pp_client_rma, async_pp_client_rma, {
    ...
    match infocap {
        prefix::InfoWithCaps::Msg(entry) => {
            ...
            let test_sizes = gl_ctx.test_sizes.clone();
            for msg_size in test_sizes {
                call!(prefix::pingpong_rma, &entry, &mut gl_ctx, &cq_type, &tx_cntr, &rx_cntr,&ep, &mut mr_desc, prefix::RmaOp::RMA_WRITE, &remote, 100, 10, msg_size, false);
            }
